...
export default function SignInClient({
    ...
    // ✅ Debug: Log the callback URL setup
    useEffect(() => {
        console.log(`[SignInClient] Component mounted with:`)
        console.log(`  - currentLocale: ${currentLocale}`)
        console.log(`  - initialCallbackUrl: ${initialCallbackUrl}`)
    ...
    const handleSubmit = async (e: React.FormEvent, isTwoFactorStep: boolean = false) => {
        ...
        try {
            ...
            // If it's encoded, decode it; if it's plain text, use as-is
            let callbackToUse = callbackUrl
            try {
                // Check if URL is encoded by looking for %
            ...
            const result = await signIn('credentials', {
                ...
                redirect: false,
                callbackUrl: callbackToUse  // ✅ Pass callback to NextAuth
            })
            ...
            if (result?.error) {
                ...
                // ✅ signIn successful
                // With redirect: false, NextAuth returns result.url with the callback redirect
                // If result.url exists, use it (NextAuth already validated it)
                // Otherwise, fallback to callbackToUse (middleware will handle role checks)
                const redirectUrl = result.url || callbackToUse
                console.log(`[SignInClient] Login successful, redirecting to: ${redirectUrl}`)

                // Force router push immediately to prevent getting stuck
                if (redirectUrl) {
    ...
    const handleGoogleSignIn = async () => {
        setLoading(true)
        try {
            // ✅ Use the same callback URL handling as credentials signin
            let callbackToUse = callbackUrl
            try {
                if (callbackUrl.includes('%')) {
                    callbackToUse = decodeURIComponent(callbackUrl)
            ...
            console.log(`[SignInClient] Google signin with callbackUrl: ${callbackToUse}`)
            await signIn('google', { callbackUrl: callbackToUse })
        } catch (error) {
